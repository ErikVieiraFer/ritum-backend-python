...
async def scrape_tjsp_process(browser: Browser, process_number: str) -> str:
    ...
    try:
        ...
        # O site do TJSP divide o número em duas partes
        parts = process_number.split('.')
        if len(parts) > 1:
            number_prefix = parts[0]
            # O resto do número, removendo o dígito verificador e o ano/tribunal
            number_suffix_parts = parts[1].split('-')
            if len(number_suffix_parts) > 1:
                number_suffix = number_suffix_parts[1]
            else:
                # Caso o formato seja inesperado, usa o número completo
        ...
        # Em caso de erro, tira um screenshot da página atual para depuração
...
# Para executar este arquivo de forma independente para teste:
# if __name__ == '__main__':
#     # Use um número de processo real do TJSP que você encontrou na Fase 1
#     test_process_number = "1000550-94.2022.8.26.0005" 
#     # This test will not work standalone anymore as it requires a browser instance
#     # result_path = asyncio.run(scrape_tjsp_process(test_process_number))
#     # print(f"Scraping concluído. Resultado salvo em: {result_path}")
...
# Define o caminho base para os templates e para os documentos gerados
...
# Esta estrutura mapeia um ID amigável para o nome do arquivo e um nome de exibição.
DOCUMENT_TEMPLATES = {
...
def get_available_templates():
    """Retorna a lista de templates disponíveis no formato especificado."""
    return [
...
def generate_document(template_id: str, client_data: dict, lawyer_data: dict, case_details: dict) -> str:
    """
    Gera um documento .docx a partir de um template, preenchendo com dados fornecidos.

    Args:
        template_id: O ID do template (ex: 'PROCURACAO_AD_JUDICIA').
    ...
    if not template_path.is_file():
        raise FileNotFoundError(f"O arquivo de template '{template_info['filename']}' não foi encontrado em {TEMPLATE_ROOT}")

    doc = DocxTemplate(template_path)

    # Prepara o contexto, combinando todos os dados recebidos
    context = {
    ...
    # Adiciona dados formatados ou compostos
